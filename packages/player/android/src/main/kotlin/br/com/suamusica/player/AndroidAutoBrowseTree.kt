...
class AndroidAutoBrowseTree() {
    ...
    // Media item storage - Now instance variables
    ...
    // Content storage - Now instance variables
    ...
    // Data caches for search - Now instance variables
    ...
    private fun buildMediaHierarchy() {
        ...
        if (albumItems.isNotEmpty()) {
            albumsMetadata.setArtworkUri(albumItems[0].mediaMetadata.artworkUri)
        }
        
        val albumsItem = MediaItem.Builder()
            .setMediaId(ALBUMS_ID)
            .setMediaMetadata(albumsMetadata.build())
            .build()
    ...
    fun createPlayableMediaItem(media: Media): MediaItem {
        val artworkUri = try {
            ...
            Log.w("AndroidAutoBrowseTree", "Invalid artwork URI: ${media.bigCoverUrl}")
            null
        }

        // Create extras bundle for additional data
        val extras = Bundle().apply {
            putString("fallbackUrl", media.fallbackUrl)
            putBoolean("isFavorite", media.isFavorite ?: false)
            putString("author", media.author)
        ...
        // Add duration if available (Android Auto uses this for UI)
        // Note: You might want to add duration to your Media class
        ...
        // Store reference to original Media object
